...
class Cat {
public:
  Cat(std::shared_ptr<Animal> a) : a_(a) {}
  Cat(Animal*) {}
  void update() { a_->name = "tom cat"; }
  std::shared_ptr<Animal> Get() { return a_; }

private:
  std::shared_ptr<Animal> a_;
};
...
int main(int argc, char *argv[]) {
  ...
  // std::cout << "\n获取指针:1 "
  //           << "addr:" << &a << " value:" << a
  //           << std::endl; // 获取指针:1 addr:0x7ff7bb9e8468
  //           value:0x600000184348
  ...
  //     << "\n重新获取指针:1 "
  //     << "addr:" << &c << " value:" << c
  //     << std::endl; // 重新获取指针:1 addr:0x7ff7bb9e8430
  //     value:0x600000184348
  ...
  /// 对象变指针 
  Animal cat("cat");
  std::cout << "name:" << cat.GetName() << std::endl;
  Cat(&cat);
  return 0;
}
